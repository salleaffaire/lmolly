...
public:
   aa_broker_debug(lmolly::aa_sync_queue<std::string> *sq) : mIsRunning(true), mpSQ(sq) {
   ...
   void broker() {
      ...
      while (mIsRunning) {
         std::this_thread::sleep_for(std::chrono::milliseconds(period_in_millisecond));
         std::stringstream ss;
         ss << "Elapsed : " << mCounter++ * period_in_millisecond << " milliseconds" << std::endl;
         //std::cout << ss.str();
         mpSQ->push(ss.str());
      }
